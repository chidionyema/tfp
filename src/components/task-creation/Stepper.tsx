...
import type { PerkItem, TaskMode, TaskFormData } from "@/types/task";
...
const modalReducer = (state: ModalState, action: { type: "OPEN"; modal: ModalKey } | { type: "CLOSE" }): ModalState => {
  switch (action.type) {
    case "OPEN":
...
const validateFormData = (data: TaskFormData, step: Step): ValidationErrors => {
  ...
  if (step === Step.Review && !data.escrowAgreed) e.escrowAgreed = "You must agree to escrow terms";
  return e;
};
...
export const Stepper: React.FC = () => {
  ...
  const validationErrors = useMemo(() => validateFormData(formData, currentStep as Step), [formData, currentStep]);
  ...
  const addPerkByType = useCallback((type: "payment" | "good" | "service") => {
  ...
  const navigatePrev = useCallback(() => setCurrentStep((s) => Math.max(Step.Basic, s - 1)), [setCurrentStep]);

  const handlePublish = useCallback(() => {
    const errs = validateFormData(formData, Step.Review);
    if (Object.keys(errs).length === 0) alert("Publishing task… 🎉 (demo)");
  }, [formData]);
  ...
  // ---------------- Draft persistence --------
  useEffect(() => {
    if (Object.keys(formData).length) localStorage.setItem("taskWizardDraft", JSON.stringify(formData));
  }, [formData]);
  ...
  return (
    <div className="bg-white rounded-xl border p-4 sm:p-6">
          ...
          {STEP_TITLES.map((t, i) => (
            <React.Fragment key={t}>
              <li className="flex flex-col items-center text-center">
                <div className={`flex items-center justify-center w-7 h-7 rounded-full border-2 text-xs ${i + 1 <= currentStep ? "bg-indigo-600 border-indigo-600 text-white" : "border-gray-300 text-gray-400"}`}>{i + 1}</div>
                <span className={`mt-1 text-xs ${i + 1 <= currentStep ? "text-indigo-600" : "text-gray-500"}`}>{t}</span>
              </li>
              {i < STEP_TITLES.length - 1 && <div className={`flex-1 h-0.5 mx-2 ${i + 1 < currentStep ? "bg-indigo-600" : "bg-gray-300"}`} />}
            </React.Fragment>
          ))}
        </ol>
      </nav>
      ...
      <div className="flex justify-between pt-6">
        <button onClick={navigatePrev} disabled={currentStep === Step.Basic} className="px-4 py-2 border rounded-lg text-sm hover:bg-gray-50 disabled:opacity-50">Previous</button>
        {currentStep < Step.Review ? (
          <button onClick={navigateNext} className="px-6 py-2 bg-indigo-600 text-white rounded-lg flex items-center gap-2 hover:bg-indigo-700">
            Next <ChevronRight size={16} />
          </button>
        ) : (
          <button onClick={handlePublish} disabled={Object.keys(validationErrors).length > 0} className="px-8 py-2 bg-green-600 text-white rounded-lg disabled:bg-gray-400">
            Publish Task
          </button>
        )}
      </div>
      ...
      {needsPhysical(formData.mode) && (
        <>
          <Modal isOpen={modalState.open === "location"} onClose={closeModal} title="Task Location">
            <LocationPicker isOpen={modalState.open === "location"} onClose={closeModal} onLocationSelect={(loc: Location) => { handleInputChange("location", loc); closeModal(); }} />
          </Modal>
          <Modal isOpen={modalState.open === "dropoff"} onClose={closeModal} title="Drop-off Location">
            <LocationPicker isOpen={modalState.open === "dropoff"} onClose={closeModal} onLocationSelect={(loc: Location) => { handleInputChange("dropoffLocation", loc); closeModal(); }} />
          </Modal>
        </>
      )}
      <Modal isOpen={modalState.open === "photos"} onClose={closeModal} title="Upload Photos">
        <PhotoUpload isOpen={modalState.open === "photos"} onClose={closeModal} onPhotosSelected={(p: UploadedPhoto[]) => { handleInputChange("photos", p); closeModal(); }} maxPhotos={5} required={false} />
      </Modal>
    </div>
  );
};

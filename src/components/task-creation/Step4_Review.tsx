...
import { MapPin, Clock, LockKeyhole } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-gray-50 rounded-lg p-4 lg:p-6">
        {/* title & description */}
        <h3 className="font-semibold text-lg text-gray-900 mb-2">{data.title || "Untitled Task"}</h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4" aria-label="Task attributes">
          {urgency && (
            <span className={`px-3 py-1 rounded-full text-xs font-medium ${urgency.color}`}>{urgency.name}</span>
          )}
          {category && (
            <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-800 text-xs font-medium">{category.name}</span>
          )}
          {data.estimatedDuration && (
            <span className="px-3 py-1 rounded-full bg-gray-100 text-gray-800 text-xs font-medium flex items-center gap-1">
              <Clock size={12} />
              {data.estimatedDuration.replace(/minutes?/, "m").replace(/hours?/, "h")}
            </span>
          )}
          <span className="px-3 py-1 rounded-full bg-green-100 text-green-800 text-xs font-medium">
            {data.perks.length} Perk{data.perks.length !== 1 ? "s" : ""} {isCombo && "(Combo)"}
          </span>
        </div>
        ...
        {data.specificRequirements && (
          <div className="mt-4">
            ...
            <p className="text-sm text-gray-700 whitespace-pre-wrap">{data.specificRequirements}</p>
          </div>
        )}
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {data.photos.map((file, idx) => {
                ...
                return (
                  <div key={idx} className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-md overflow-hidden border">
                    <Image src={url} alt={`Task photo ${idx + 1}`} fill className="object-cover" onLoadingComplete={() => URL.revokeObjectURL(url)} />
                  </div>
                );
              })}
            </div>
        ...
        {/* perk package */}
        {data.perks.length > 0 && (
          <div className="mt-6 border rounded-lg bg-white p-4" aria-labelledby="perk-heading">
            ...
            <ul className="space-y-3">
              ...
              <li className="pt-3 mt-3 border-t text-sm text-gray-700">
                ...
                <div className="flex justify-between font-medium mt-1">
                  ...
                  <span className="text-green-600">£{(totalValue - platformFee).toFixed(2)}</span>
                </div>
              </li>
            </ul>
          </div>

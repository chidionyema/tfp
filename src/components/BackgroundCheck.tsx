...
export const BackgroundCheck: React.FC<BackgroundCheckProps> = ({
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // IMPORTANT: We must supply personalInfo.level here (default to "none") so that
  // the `level: DbsLevel` field is never missing. Otherwise TS will complain.
  // ─────────────────────────────────────────────────────────────────────────────
  const [formData, setFormData] = useState<BackgroundCheckData>({
    ...
    personalInfo: {
      ...
      // ← Add `level` here (default: "none") to satisfy the required `DbsLevel` field:
      level: initialData?.personalInfo?.level || "none",
      address: {
  ...
  const [currentStep, setCurrentStep] = useState<
    "country" | "info" | "documents" | "consent" | "review"
  >("country");
  ...
  // Use validation hook (validates personal Info step)
  ...
  const handleCountryChange = useCallback((country: string) => {
    setFormData((prev) => ({
      ...
      personalInfo: {
        ...prev.personalInfo,
        nationalId: "",
        // Reset state/county/etc. whenever country changes:
        address: {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  //  handleNext: when you click “Continue”
  // ─────────────────────────────────────────────────────────────────────────────
  const handleNext = useCallback(() => {
    ...
    const stepOrder: Array<"country" | "info" | "documents" | "consent" | "review"> = [
      "country",
      "info",
      "documents",
      "consent",
  ...
  // handleBack: move one step backward
  ...
  // handleSubmit: when you click “Submit Background Check” on final step
  ...
  return (
    <div className={`max-w-4xl mx-auto p-4 sm:p-6 bg-white dark:bg-gray-900 ${className}`}>
      ...
      <PhotoUpload
        ...
        description={`Please upload clear photos of your documents as required for ${
          countryConfig.name
        } background checks`}
        maxPhotos={10}
        maxSizePerPhoto={15}
